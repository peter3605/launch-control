...
#
#   ./migrate.sh <repo-path>            dry run: report what WOULD change
#   ./migrate.sh <repo-path> --apply    actually change it
#
# THIS IS NOT THE INSTALLER. Setting a machine up for the first time is ./setup.sh.
# This script is for the narrow case of a repo that predates the plugin and still
# has Launch Control's commands and hooks COPIED into its own .claude/ directory:
# it retires those copies, splits the private notice out, and stamps the drift
# baseline. On a repo that never had them there is nothing here to do. It was
# called install.sh until 0.6.0, which is why a stranger kept running it first.
#
...
if [ -z "$TARGET" ] || [ ! -d "$TARGET" ]; then
  echo "usage: ./migrate.sh <repo-path> [--apply]" >&2
  echo "       (setting up a machine for the first time? that is ./setup.sh)" >&2
  exit 2
fi
...
if [ -f "$TARGET/CLAUDE.md" ]; then
  ...
  # Check any edit with /bin/bash -n migrate.sh, not whatever bash is on PATH.
...
say "Next:"
say "  ./setup.sh            checks the plugin and the Notion connector are in place"
say "  ./setup.sh --apply    adds whatever is missing"
