...
This is the front door. Every other command starts from an existing story; this one starts from a document or a repo that the board has never seen. It provisions a **project inside an existing board** - the Projects row, its road view, the repo's config. It does not create the board itself (databases, properties, the shared views); if there is no board, say so, point at `docs/notion-schema.md`, and stop.
